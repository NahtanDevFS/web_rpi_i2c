...
function App() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // CORRECCION: Se ajusto el nombre de la variable de la URL base
    ...
    const intervalId = setInterval(fetchData, 2000); // Refresca cada 2 segundos
    ...
    <div className="app-container">
      ...
      <section className="status-panel">
        <div className="status-item">
          ...
          >
            {isConnected ? "Estable" : "Desconectado"}
          </span>
        </div>
        <div className="status-item">
          <span className="status-label">Distancia:</span>
          <span className="distance-value">{sensorData.distancia} mm</span>
        </div>
      </section>

      <main className="lcd-controls">
        {/* Formulario para LCD 1 */}
      ...
      <footer className="last-updated">
        Última actualización: <span id="last-updated">{lastUpdated}</span>
      </footer>
    </div>
  );
}
